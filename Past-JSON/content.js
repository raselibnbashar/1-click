    ...
    try {
        ...
        if (site.includes("https://ln.bdjobs.com/linkedinApplication/jobListLn.asp?")) {
            ...
            let mobileNumber = document.querySelector('input[name="contact_phone"]');
             ...
             let countryElement = document.querySelector('select[name="contact_country"]');

            // Iterate through the options to find the matching country and set its value
            for (let i = 0; i < countryElement.options.length; i++) {
            if (countryElement.options[i].text.includes(dataVal.countryName)) {
                countryElement.value = countryElement.options[i].value;
